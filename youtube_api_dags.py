...
with DAG(
    dag_id = "youtube_data_ingestion",
    start_date = datetime(2025, 10, 5),
    schedule_interval= '@hourly',
    catchup = False,
) as dag:
    fetch_channel_data_task = PythonOperator(
        task_id = 'fetch_channel_data',
        python_callable = fetch_channel_data
    )
    fetch_video_data_task = PythonOperator(
        task_id = 'fetch_video_data',
        python_callable = fetch_video_data
    )

    fetch_channel_data_task >> fetch_video_data_task
